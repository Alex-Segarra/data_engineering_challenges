...
def delete_and_start_over(dbname):
    """
  PURPOSE: delete and start over the database.
  ...
  OUTPUT:
    ...
    if isinstance(dbname, list):
        for d in list(dbname):
            print(f'Getting rid of database {d}...')
            subprocess.getoutput(f'''sudo -u postgres psql -c "DROP DATABASE {d};"''')
    else:
        subprocess.getoutput(f'''sudo -u postgres psql -c "DROP DATABASE {dbname};"''')
        
    subprocess.getoutput(f'''rm -r data/''')
    
    subprocess.getoutput(f'''rm config.json''')
    
    subprocess.getoutput(f'''sudo  apt-get -y purge postgresql*''')